	...
	ColumnToggle.prototype.init = function() {
		...
		$( this.headers ).not( "td" ).each( function() {
			...
			if( priority && priority !== "persist" ) {
				...
				if($menu.find('#hide-all').length == 0){
					$("<label><input id='hide-all' type='checkbox'>OCULTAR TODOS</label>" )
						.appendTo( $menu )
						.children( 0 );
				}
		...
		$menu.find( 'input[type="checkbox"]' ).on( "change", function(e) {
			...
			var attr = e.target.attributes["id"];
			if(typeof attr === typeof undefined || attr === false){
				...
				var inputs = $menu.find( 'input[type="checkbox"]');
				var hide = inputs[0].checked;
				for(var i=0;i<inputs.length;i++){
					var attr = inputs[i].attributes["id"];
